...
@app.route("/api/ui_profile", methods=["POST"])
def ui_profile():
    try:
        data = request.get_json(force=True)
    except Exception:
    ...
    if not language_code:
        return jsonify({"ok": False, "error": "language_code manquant."}), 400

    prompt = f"""
Tu es un assistant pour une plateforme interne appelée IDEA.

On te donne :
- un champ "language_code" (code ISO ou nom de langue)

language_code = "{language_code}"

OBJECTIF — Traduction UI (page profil)
-------------------------------------
Tu dois traduire FIDÈLEMENT le bloc français ci-dessous
dans la langue correspondant à language_code.

Règles STRICTES :
- Tu ne dois RIEN inventer
- Tu ne dois PAS ajouter de texte
- Tu dois conserver le sens et la structure
- Tu dois garder les symboles (◀, ·)
- Tu réponds UNIQUEMENT avec un JSON valide

Bloc français de référence :

title_fr  : "On démarre par toi"
intro_fr  : "Avant de commencer, indique simplement qui tu es, où tu travailles et quel est ton rôle."
